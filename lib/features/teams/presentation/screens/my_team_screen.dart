  ...
  Widget _buildMemberCard(UserModel member) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(isOwner ? 'Капитан команды' : 'Участник'),
        trailing: isOwner ? null : IconButton(
          icon: const Icon(Icons.remove_circle_outline, color: AppColors.error),
          onPressed: () => _removeMemberFromTeam(member),
        ),
                ...
                child: ListTile(
                  leading: CircleAvatar(
                    ...
                    child: friend.photoUrl == null 
                        ? Text(_getInitials(friend.name), style: const TextStyle(color: AppColors.primary))
                        : null,
                  ),
                  title: Text(friend.name),
                  subtitle: Text('Рейтинг: ${friend.rating.toStringAsFixed(1)}'),
                  onTap: () {
